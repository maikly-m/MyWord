//
// Created by 李泽 on 2020/6/16.
//

#include "crc.h"

const int crc_table_len = 256;

static unsigned short ccitt_table[crc_table_len] = {
...
extern unsigned int crc16(const unsigned char *data, int len) {
  ...
  for (int i = 0; i < len; i++) {
    ...
    dat = crc >> 8;
    crc = (crc << 8) & 0xffff;
    crc = (crc ^ ccitt_table[dat ^ (b & 0x00ff)]) & 0xffff;
  }
  return crc;
}
...
uint16_t _crc_16(uint8_t *data, uint16_t len){
